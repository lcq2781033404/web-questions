...
- e. 可以发现，无论是**新后旧前**还是**新前旧后**，都是在**真实DOM**中**移动旧节点到新节点索引**上面。
- f. 如果四种匹配没有一对是成功的，那么**遍历 oldChild **， **新前节点** 挨个和**旧节点**匹配：
    ...
    - f2. 如果没有匹配成功，说明这个新前节点在DOM中没有，是**新增节点**，那么就在DOM新建一个元素，将这个新节点插入到**旧前对应节点的前面**，然后**新前节点指针向中间移动**。
...
- g. 这个匹配过程的结束有两个条件：
    - g1. oldS > oldE 表示 oldCh 先遍历完，那么就将多余的 vCh 根据index添加到dom中去 
