...
答：利用**闭包**缓存实例对象，每次实例化的时候都判断实例对象是否存在，如果存在则不实例化了。

##### ③ 应用场景
适用于全局只存在一个实例的场景：

- 全局只存在一个的超级管理员
- vuex就是使用的单例模式，即**全局只有一个Store对象**

#### （2）策略模式
##### ① 定义
定义一系列的算法，把它们一个个封装起来，并且使它们可以相互替换。
##### ② 核心
将**算法的使用**和**算法的实现**分离开来。**目的就是为了减少if-else语句的使用次数！！！**
...
##### ③ 应用场景
该模式应用场景很多，绝大多数功能都可以用这种**使用**和**实现**分离的模式来做：

- **表单校验**，我们把表单的验证规则都写到一起（**算法的实现**），然后在用到的地方根据需求进行调用（**算法的使用**）。
- vue的**生命周期钩子函数**就是利用了策略模式实现的，把多个生命周期放到一起集中定义，然后在各个阶段触发

#### （3）模板模式
##### ① 定义
模板模式由两部分结构组成，第一部分是抽象父类，第二部分是具体的实现子类。
##### ② 核心
模板模式的核心就是**用接口或者抽象类规范类的实现**
##### ③ 应用场景
- 在ts开发中，用接口定义对象或类的类型，就是模板模式的体现
